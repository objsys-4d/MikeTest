...
$vlCounter:=0
For each ($loopCountry; $colCountries)
	
	
	$tableName_t:="Humans_"+$loopCountry
	
	//$tablePtr:=Get pointer("["+$tableName_t+"]")
	
	$esRecordsCountry:=dsImport.Records.query("country_code = :1"; $loopCountry)
	
	For each ($loop; $esRecordsCountry)
